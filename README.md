...
## Winter Vacation Study Notes

### 项目结构
```
WINTER_VACATION_STUDY_NOTES
├── Code Practice/          # 代码实战
...
### 项目结构
```
ADDITIONAL_STUDY_NOTES
├── Code Practice/          # 核心实战：基于Qwen3模型的微调
│   └── Qwen3_PEFT/         # 以Qwen3-4b为基座模型的参数高效微调
│     ├── SFT/              # 监督微调（基于LoRA）：指令微调实战
│     ├── PPO/              # 近端策略优化
│     └── GRPO/             # 群组相对策略优化
├── Basic Knowledge/        # 基础知识：LLM开发基础手册
│   ├── Docker基础使用.md    # Docker快速搭建深度学习环境、基础命令及常见报错解决
│   ├── 命令行基础工具.md     # Linux/macOS高频命令、脚本编写技巧
│   └── 开发基础工具.md       # Conda/Pip/Uv（Python环境管理） + Git（版本控制）
└── README.md
```

#### 1. LLM 微调技术
- **SFT (Supervised Fine-Tuning)**：基于 **LoRA（低秩适应）** 技术，以较低的成本让模型学会遵循指令，完成特定领域的对话能力微调。
- **PPO (Proximal Policy Optimization)**：经典的 **RLHF（基于人类反馈的强化学习）** 算法。通过奖励模型进一步优化模型行为，使其生成更符合人类偏好的内容。
- **GRPO (Group Relative Policy Optimization)**：由 **DeepSeek** 提出的创新算法（PPO 的变体/替代方案）。通过组内评分对比替代传统的 Critic 模型，在保持效果的同时提升了训练效率。

#### 2. 开发基础手册
- **命令行基础工具**：涵盖 **Linux/macOS** 下的高频开发指令，以及 **Apt-get** 包管理工具的使用技巧，提升终端操作效率。
- **开发基础工具**：
  - **环境管理**：`conda` 与 `pip` 管理 Python 环境和包依赖。
  - **项目管理**：`uv` 工具进行现代化 Python 项目与依赖管理。
  - **版本控制**：`git` 常用指令与协作流程，保障代码的可追溯性。
- **Docker 基础使用**：包含 **Docker 快速配置深度学习环境**（如 PyTorch 镜像）、**基础操作命令**（容器生命周期管理）以及**常见报错解决方案**，确保环境的一致性与可复现性。
