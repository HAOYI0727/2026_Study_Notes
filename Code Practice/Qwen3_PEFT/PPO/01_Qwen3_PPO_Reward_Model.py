"""
Name: Qwen_PPO_Reward_Model.py
Description: 基于Qwen3-4B-Instruct模型的奖励模型(Reward Model)训练完整实现
             包含4-bit量化、LoRA参数高效微调、prepare_model_for_kbit_training预处理、RewardTrainer训练、模型测试和保存的完整流程
"""

"""
奖励模型在RLHF中的核心作用：
    1. 学习人类偏好：通过偏好数据对(chosen/rejected)学习人类偏好
    2. 提供奖励信号：为PPO/GRPO强化学习提供连续的奖励分数
    3. 替代人工评估：实现自动化、可扩展的模型评估
    4. 细粒度反馈：相比离散的偏好标签，提供更细致的质量评估
"""
...
RewardTrainer vs 普通Trainer的优势：
    ...
    2. 专用损失函数：使用Pairwise Ranking Loss
...
"""
prepare_model_for_kbit_training函数：
使用原因：
    1. 梯度计算兼容性：4-bit量化模型需要特殊处理才能正确计算梯度
    2. 梯度检查点：自动启用gradient_checkpointing，用时间换空间
    3. 输入数据类型：确保输入数据类型与量化参数兼容
    4. 参数冻结：正确处理量化后的参数冻结状态
主要作用：
    - 启用梯度检查点以节省显存
    - 将线性层转换为可在低精度下训练的版本
    - 设置模型为训练模式
...
# 创建LoRA模型
# get_peft_model会冻结原始模型参数，只在指定模块插入可训练的LoRA参数
...
reward_config = RewardConfig(
    output_dir="./qwen3_reward_lora",          # 模型和检查点的输出目录
...
save_path="./Qwen3_PPO_Reward_Model"
# 保存LoRA模型权重（只保存可训练参数）
