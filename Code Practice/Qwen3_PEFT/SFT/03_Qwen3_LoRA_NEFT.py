"""
Name: Qwen3_LoRA_NEFT.py
Description: 基于Qwen3-4B-Instruct模型的LoRA指令微调实现，包含手动的损失掩码逻辑，
             并添加NEFT (Noisy Embedding Fine-Tuning) 噪声训练技术，通过向嵌入向量添加噪声来增强模型泛化能力，防止过拟合。
"""
...
"""
NEFT (Noisy Embedding Fine-Tuning) 原理：
    通过在嵌入向量中添加均匀分布噪声，增强模型的泛化能力，防止过拟合。
噪声公式：
    e' = e + noise, where noise ~ Uniform(-mag_norm, mag_norm)
    mag_norm = alpha / sqrt(dims)，其中dims = sequence_length * embedding_size
参数建议：
    alpha通常在5-15之间，对大多数任务效果最好。
"""
neftune_noise_alpha = 10.0  # 噪声强度超参数，值越大噪声越强
...
for epoch in range(epochs):
    for step, (inputs, loss_mask) in enumerate(data_loader):
        ...
        input_ids = inputs.pop("input_ids").to("cuda")  # 形状：[batch_size, sequence_length]
        # 获取attention_mask并移到GPU，用于指示哪些位置是有效输入
        attention_mask = inputs["attention_mask"].to("cuda")  # 形状：[batch_size, sequence_length]
        # 将loss_mask移到GPU，用于只计算需要训练部分的损失
        loss_mask = loss_mask.to("cuda")  # 形状：[batch_size, sequence_length]
        
        # ----- NEFT噪声添加步骤 -----
        # 1. 获取嵌入层输出
        # Qwen3模型在PEFT包装后的嵌入层访问方式：model.model.model.embed_tokens
        # PEFT包装后模型结构为 PeftModel -> 原始模型 -> embed_tokens
        ...
        # 2. 计算噪声的幅度
        # dims = sequence_length * embedding_size，即嵌入向量的总维度
        dims = torch.tensor(input_embeddings.size(1) * input_embeddings.size(2))
        # 噪声幅度计算公式：mag_norm = alpha / sqrt(dims)，确保噪声在不同维度大小的模型上保持相对一致的影响
        mag_norm = neftune_noise_alpha / torch.sqrt(dims)
        
        # 3. 生成均匀分布噪声
        # 生成与嵌入向量相同形状的均匀分布噪声，范围在[-mag_norm, mag_norm]之间
        noise = torch.zeros_like(input_embeddings).uniform_(-mag_norm, mag_norm)  # 形状：[batch_size, sequence_length, embedding_size]
        
        # 4. 将噪声添加到原始嵌入向量中，实现噪声增强
        input_embeddings = input_embeddings + noise  # 形状保持不变：[batch_size, sequence_length, embedding_size]
        
        # 5. 将带噪声的嵌入向量添加到输入字典中，这样模型会使用带噪声的嵌入向量而不是原始的input_ids进行前向传播
        # 注意：当提供inputs_embeds时，模型会忽略input_ids参数
        inputs["inputs_embeds"] = input_embeddings
        # 确保attention_mask也在输入字典中（用于模型的注意力机制）
        inputs["attention_mask"] = attention_mask
        
        # 前向传播：使用带噪声的嵌入向量
        outputs = model(**inputs)  # 由于提供了inputs_embeds，模型不会使用input_ids
...
save_path="./03_Qwen3_LoRA_NEFT_model"
...
核心思想：
   ...
   - 引入NEFTune噪声正则化技术，增强模型泛化能力
   - 保持system提示和用户问题不变，专注于优化回答质量

NEFT技术优势：
   - 简单有效：只需在嵌入层添加噪声，无需修改模型结构
   - 防止过拟合：噪声增加了训练数据的多样性，类似于数据增强
   - 提升生成质量：论文表明NEFTune能显著提高对话模型的生成质量
   - 计算开销小：噪声生成和添加的计算成本几乎可以忽略

实现重点：
   - 损失掩码机制：通过识别"<|im_start|>assistant"标记，精确定位回答区域
   - LoRA适配：在注意力层和MLP层添加低秩矩阵，实现高效微调
   - NEFT噪声：动态计算噪声幅度，确保尺度适当
   - 嵌入层访问：正确处理PEFT包装后的模型结构

使用方法：
   - 准备对话数据：每个样本包含system、user、assistant三部分
   - 调整超参数：
     * LoRA参数：r（秩）、lora_alpha（缩放因子）控制微调强度
     * NEFT参数：neftune_noise_alpha（5-15）控制噪声强度
     * 学习率：建议使用较小的学习率（1e-5到5e-5）
